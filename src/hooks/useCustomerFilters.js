  ...
  return useMemo(() => {
    ...
    const comparator = (a, b) => {
      ...
      for (const key of priorityList) {
        ...
        // Prioritize filtered values (e.g., hasLoan, hasDeposit, categories)
        if ((key === 'hasLoan' || key === 'hasDeposit') && filters[key]) {
          const target = filters[key];
          const aMatch = a[key] === target ? 0 : 1;
          const bMatch = b[key] === target ? 0 : 1;
          diff = aMatch - bMatch; // matches first
        }

        if (key === 'categories' && filters.categories && filters.categories.length > 0) {
          const aMatch = filters.categories.includes(a.category) ? 0 : 1;
          const bMatch = filters.categories.includes(b.category) ? 0 : 1;
          diff = aMatch - bMatch;
        }
